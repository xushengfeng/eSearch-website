...
const allStrFile = args.includes("--all") ? args[args.indexOf("--all") + 1] : "";
const getAllAllStr = args.includes("--auto-clean-trans");

const sourcePath = path.join(rootDir, "source.json");

console.log(`latestSrcId: ${execSync(`git log -n 1 --pretty=format:%h -- ${sourcePath}`)}`);
...
function getToTranslate(lang) {
    ...
    /**@param {string} i  */
    function addItem(i) {
        if (!enableEn) t.push(`"${source[i]}","${i}","${pure ? "" : l[source[i]] || ""}"`);
        else t.push(`"${source[i]}","${i}","${enLan[source[i]] || ""}","${pure ? "" : l[source[i]] || ""}"`);
        tt.push(i);
    }

    if (!w) {
        const commit = srcCommit[lang];
        const diff = commit.id ? execSync(`git diff ${commit.id} master ${sourcePath}`).toString() : "";
...
if (ai) {
    ...
    console.log(tt);
    if (!process.env.gpt) throw new Error("Please set GPT_TOKEN: export gpt=sk-");
    if (tt.length > 0)
        fetch("https://api.chatanywhere.tech/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: "system",
                        content: "你将翻译提供的JSON，不需要任何解释，$1,$2等用于变量占位，请保留",
                    },
                    {
                        role: "user",
...
function extractStringsFromCode(code) {
    ...
    while ((match = STRING_REGEX.exec(code)) !== null) {
        ...
        if (t.match(/[a-z]/) || t.match(/^[A-Z ]*$/) || t.match(/[0-9]/) || t.includes(".") || t.startsWith("#"))
            continue;
        if (t.trim() === "") continue;
        strings.push(t);
    }
...
if (getAllStr) {
    ...
    if (!allStrFile) console.log(unAdd.map((i) => `${i}    ${Array.from(strs2files.get(i)).join(", ")}`).join("\n"));
